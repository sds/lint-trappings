module LintTrap
  ...
  class Lint
    # @return [LintTrap::Linter, nil] linter that reported the lint (if applicable)
    attr_reader :linter

    # @return [String] file path to which the lint applies
    attr_reader :path

    # @return [Range<LintTrap::Location>] source range of the problem within the file
    ...
    # @param options [Hash]
    # @option options :linter [LintTrap::Linter] optional
    # @option options :path [String]
    # @option options :source_range [Range<LintTrap::Location>]
