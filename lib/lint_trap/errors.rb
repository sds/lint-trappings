...
module LintTrap
  # Abstract error. Separates LintTrap errors from other kinds of
  # errors in the exception hierarchy.
  #
  # @abstract
  class LintTrapError < StandardError
      ...
      if args.any?
        ...
        if @exit_status
          ...
          ancestors[1..-1].each do |ancestor|
            return 70 if ancestor == LintTrapError # No exit status defined
  ...
  # @abstract
  class ConfigurationError < LintTrapError
    exit_status 78 # EX_CONFIG
  end

  # Raised when a LintTrap::Application subclass does not set a value for a
  ...
  # @abstract
  class UsageError < LintTrapError
  ...
  class LinterError < LintTrapError; end
  ...
  class ParseError < LintTrapError
    # @return [String] path to the file that failed to parse
    attr_reader :path

    # @return [Range<LintTrap::Location>] source range of the parse error
