...
module LintTrap::Formatter
  ...
  class Default < Base
    def started(files_to_lint)
      files = LintTrap::Utils.pluralize('file', files_to_lint.count)
    ...
    def finished(report)
      ...
      files = LintTrap::Utils.pluralize('file', report.documents_inspected.count)
      output.print "#{report.documents_inspected.count} #{files} inspected"

      if report.failures?
        failures = LintTrap::Utils.pluralize('failure', report.failures.count)
        output.print ', '
        output.error "#{report.failures.count} #{failures} reported", false
      end

      if report.warnings?
        warnings = LintTrap::Utils.pluralize('warning', report.warnings.count)
        output.print ', '
