...
RSpec.describe LintTrap::FileFinder do
  describe '.find' do
    let(:files) { [] }
    ...
    context 'when included_paths contains a non-existent file/directory' do
      ...
      it 'raises' do
        expect { subject }.to raise_error LintTrap::InvalidFilePathError
      end
    end
    ...
    context 'when included_patterns matches no file' do
      ...
      it 'raises' do
        expect { subject }.to raise_error LintTrap::InvalidFilePatternError
      end
    end
